...
2. **Branch Pattern**: Recognizes `buddy/*` branches
...
```typescript
const branchName = `buddy/update-${group.name.toLowerCase().replace(/\s+/g, '-')}-${timestamp}`

// Examples:
// buddy/update-non-major-dependencies-1704123456789
// buddy/update-react-ecosystem-1704123456789
// buddy/update-security-patches-1704123456789
```
