...
export function formatPretty(result: AuditResult, opts: { color?: boolean } = {}): string {
  ...
  if (result.workflows.length === 0)
    return c(GRAY, 'buddy security: no workflows found under .github/workflows.')

  if (result.findings.length === 0)
    return c(GREEN, `buddy security: ${result.workflows.length} workflow${result.workflows.length === 1 ? '' : 's'} clean.`)
