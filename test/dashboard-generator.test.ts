...
`
  ...
  it('should extract package names from Renovate PR title formats', () => {
    ...
    expect(result.body).toContain('(`phpunit/phpunit`)')
    expect(result.body).toContain('(`symfony/console`)')

    // Should include rebase branch comments
    expect(result.body).toContain('<!-- rebase-branch=renovate/phpunit-phpunit-12.x -->')
